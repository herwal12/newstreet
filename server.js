...
const STAFF_CHANNEL_ID = '1531118597345120447';
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isModalSubmit()) {
        ...
        let posteNom = "Inconnu";
        if (originalEmbed && originalEmbed.title) {
            const parts = originalEmbed.title.split('—');
            if (parts[1]) {
                posteNom = parts[1].trim();
            }
        }
        ...
        if (interaction.customId.startsWith('modal_accept_')) {
            ...
            try {
                ...
                const acceptEmbed = new EmbedBuilder()
                    ...
                    .setThumbnail(LOGO_URL)
                    .addFields(
                        { name: '🎫 Recrutement', value: `Votre demande de recrutement pour le poste de **${posteNom}** vient d'être revue.` },
                ...
                const fields = updatedEmbed.data.fields.map(field => {
                    if (field.name === 'Statut du Dossier') {
                        return { name: `✅ Candidature ${posteNom} Acceptée — Consignes`, value: '```\n' + reason + '\n```', inline: false };
                    }
                    return field;
                });
                ...
                await interaction.update({ 
                    content: `Candidature **${posteNom}** acceptée par ${interaction.user}`, 
                    embeds: [updatedEmbed], 
            ...
            try {
                ...
                const refuseEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: '💬 Message', value: `Bonjour. Nous vous informons que votre candidature pour le poste de **${posteNom}** chez **NewStreet Roleplay** n'a malheureusement **pas été retenue**.\n\nMerci pour l'intérêt que vous portez à notre serveur.` }
                    )
                    .setFooter({ text: 'NewStreet Roleplay — Système de Recrutement • Tous droits réservés' });
                ...
                const fields = updatedEmbed.data.fields.map(field => {
                    if (field.name === 'Statut du Dossier') {
                        return { name: `❌ Candidature ${posteNom} Refusée — Raison`, value: '```\n' + reason + '\n```', inline: false };
                    }
                    return field;
                });
                updatedEmbed.setFields(fields);
                ...
                await interaction.update({ 
                    content: `Candidature **${posteNom}** refusée par ${interaction.user}`, 
                    embeds: [updatedEmbed], 
                    components: [disabledRow] 
                });
